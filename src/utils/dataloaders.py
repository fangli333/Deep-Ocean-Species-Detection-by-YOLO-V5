...
class LoadImages:
    ...
    def __init__(self, start_time, end_time, path, img_size=640, stride=32, auto=True):
        files = []
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            while (True):
                ...
                ts = f'{hours:.0f}' + ':' + f'{minutes:.0f}' + ':' + f'{seconds:.0f}' + ':' + f'{milliseconds:.0f}'
                if (self.start_time == "00:00:00" and self.end_time == "00:00:00"):
                    while not ret_val:
                        self.count += 1
                        self.cap.release()
                        ...
                        self.frame += 1
                    s = f'video {self.count + 1}/{self.nf} ({self.frame}/{self.frames}) {path}:[{ts}] '
                    # Padded resize
                    ...
                    # 选择性处理
                    t = f'{hours:.0f}'.zfill(2) + ':' + f'{minutes:.0f}'.zfill(2) + ':' + f'{seconds:.0f}'.zfill(2) + ':000'
                    
                    # 字典序比较
                    if t < self.start_time + ":000" or t > self.end_time + ":000":
                        # print(t)
                        while not ret_val:
                            self.count += 1
                            self.cap.release()
